...
const Product = () => {
  const [ProtienFoods, setProtienFoods] = useState([]); // my data array
  ...
  console.log("ProtienFoods", ProtienFoods); // My data Array
  ...
  const getData = async () => {
    let resProtienFoods = await axios.get(
      `http://localhost:8080/Protien-Foods?product_num_ratings_gte=${Review}&product_star_rating_gte=${Rating}&product_price_gte=${maxPrice}&_page=${PageNo}&_limit=${DataLimit}`
    );
    setProtienFoods(resProtienFoods.data);
  };
